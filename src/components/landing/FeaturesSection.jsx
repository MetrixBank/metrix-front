...
import { BarChart3, Package, Users, Target, Bot, CreditCard, Wallet, GraduationCap } from 'lucide-react';
...
const FeatureCard = ({ icon: Icon, title, description, color, index }) => {
  return (
    <motion.div
      className="relative p-6 rounded-2xl border border-white/5 bg-black/40 backdrop-blur-xl overflow-hidden group hover:border-white/10 transition-colors duration-300"
      variants={{
    ...
    >
      ...
      <div className={`p-3 rounded-xl bg-white/5 w-fit mb-4 ${color} group-hover:scale-110 transition-transform duration-300`}>
        <Icon className="w-6 h-6" />
      </div>
      
      <h3 className="text-lg font-bold text-white mb-2 group-hover:text-primary transition-colors duration-300">{title}</h3>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          <h2 className="text-3xl md:text-5xl font-bold tracking-tight text-white mb-6">
            Ferramentas para <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-emerald-400">Alta Performance</span>
          </h2>
